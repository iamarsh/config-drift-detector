...
export default function RDSDriftsPage() {
  ...
  // Filter for RDS resource type only
  const rdsFilters = useMemo(() => ({
    ...filters,
    type: 'RDS',
  }), [filters])
  ...
  useEffect(() => {
    const channel = supabase
      .channel('drift_events_changes_rds_page')
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        (payload) => {
          ...
          if (newDrift.severity === 'HIGH' || newDrift.severity === 'CRITICAL') {
            addToast({
              title: `${newDrift.severity} RDS Drift Detected!`,
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-text-primary">RDS Drift Events</h2>
          <p className="mt-1 text-sm text-text-secondary">
            Configuration drift for RDS database instances
          </p>
        </div>
        ...
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-surface p-4 rounded-lg border border-border">
            <div className="text-sm text-text-secondary">Total RDS Drifts</div>
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-sm text-text-secondary">Loading RDS drifts...</p>
          </div>
        ) : (
